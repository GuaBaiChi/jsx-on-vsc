...
<!-- So it states with a useEffect triggered on url change. That should be called at least once on initial load
when that happens it calls pokeFun
In pokeFun it calls the initial API that you've already written code to do
the only difference is what happens next
it calls getPokemon
which uses a map to make another axios call for each pokemon in the list
so that ends up triggering like 20 more API calls -->